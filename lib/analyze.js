...
function housenumber( num ){
  ...
  var number = num.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  ...
  //remove common vietnamese label
  number = number.replace(/(nha so|ns|so nha|sn|lo so)/g, '');
  number = number.replace(/(so|number|no|s|lo)/g, '');
  ...
  // imported for Vietnamese
  else if( number.match(/^([0-9]+)([\/|-])([0-9]+)([\/|-])([0-9]+)$/) ){
    // split the components to attempt more advanced parsing
    var split = number.match(/^([0-9]+)([\/|-])([0-9]+)([\/|-])([0-9]+)$/);
    var house1 = split[1], delim1 = split[2], house2 = split[3], delim2 = split[4], house3 = split[5];
    if( delim1 === delim2 && delim2 === '-' ) { number = house2; }
  }
    ...
    if( split ){
      ...
      // handle small ranges, eg: '72-74' -> '72'
      // edited for Vietnamese
      else if( delim === '-' ){ number = house; }
    }
  }
